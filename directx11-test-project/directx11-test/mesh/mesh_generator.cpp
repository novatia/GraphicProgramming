...
xtest::mesh::MeshData  xtest::mesh::GenerateTorus(float innerRadius, float outerRadius, uint32 sides, uint32 rings) {
	MeshData mesh;
	const float phiStep = XM_PI / sides;
	const float thetaStep = 2.0f*XM_PI / rings;
	float pipi = 3.14159265359f * 2.0f;


	//vertices
	for (uint32 seg = 1; seg <= rings - 1; ++seg)
	{
		float phi = seg * phiStep;
		int currSeg = seg;
		...
		//2p #seg/segments
		float t1 = (float)currSeg / sides * pipi;

		MeshData::Vertex center;
		center.position.x = cos(t1) *  (innerRadius + (outerRadius - innerRadius)/2);
		center.position.y = 0;
		center.position.z = sin(t1) *  (innerRadius + (outerRadius - innerRadius) / 2);
		...
		for (int side = 0; side <= sides; side++)
		{
			...
			XMVECTOR r1f = XMLoadFloat3(&r1.position);
